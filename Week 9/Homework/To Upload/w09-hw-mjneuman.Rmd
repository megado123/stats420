...
#not needed to create a new dataframe, but given the interaction terms, I think this is a bit cleaner/more clear
#to deal with
...
Both methods appear to not have an issue (using this particular model and this set of predictors and beta values) with including false negatives - incorrectly excluding from the model. 
...
As shown in the table above it still appears that the AIC method has a higher rate of false positives, incorrectly including variables into the model for this particular model. In this simulation we can see that the BIC method actually has a higher rate of false negatives than the AIC rate. This could be due to the fact that the predictors were highly correlated with other predictors.  This would suggest that if the predictors are highly correlated,  BIC will produce higher false negatives, however it will still produce a lower rate of false positives than the AIC method.  It appears false negatives are introduced when predictors are highly correlated.
