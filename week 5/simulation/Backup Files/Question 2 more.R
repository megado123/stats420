...
simulate2 = function(num_sims, sigma){
  ...
  for(i in 1:num_sims) {
    ...
    #for clean-up later after I have this figured out.
...
eps = rnorm(sample_size_total, mean = 0 , sd = 1)
head(eps)
eps
mean(eps)

myresult = simulate2(500, 1)


View(myresult)

data = myresult[(myresult$winner == 5), ]
nrow(data)
...
head(myresult)
#create plot shows how the average train and test RMSE chances
#as a function of the model size
#also show the number of times the model of each size was chosen for each value of sigma
