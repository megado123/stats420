...
transformationsLog = function (response, predictors, dataset, responsevector){
  ...
  quad_RMSE[i]                    = rep(0, length(predictors) )
  quad_bp_p_value[i]              = rep(0, length(predictors) )
  quad_shapiro_p_value[i]         = rep(0, length(predictors) )
...
#  quad = c(quad_RMSE, quad_bp_p_value, quad_shapiro_p_value)
  ...
  #transform   = c("log", "^2 obeying hierachy", "^3 obeying hierachy")
  ...
  #transform = transform, 
  data.frame( bp = bp, shapiro = shapiro, RMSE = RMSE )
